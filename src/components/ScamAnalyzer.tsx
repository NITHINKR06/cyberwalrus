...
export default function ScamAnalyzer() {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      setTextInput(response.extractedText); // Populate the text area
      setActiveTab('text'); // Switch to the text tab
      toast.success('Text extracted from image! Now analyze it.');
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
        ...
        {activeTab === 'text' && (
          <div>
            <textarea value={textInput} onChange={(e) => setTextInput(e.target.value)} placeholder={t('scamAnalyzer.textPlaceholder')} className="w-full h-40 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" />
        ...
        {activeTab === 'url' && (
          <div>
            <input type="url" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} placeholder={t('scamAnalyzer.urlPlaceholder')} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
        ...
        {activeTab === 'image' && (
          <div>
            ...
            <div
              ...
              className="w-full h-40 flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50"
            >
              {imagePreview ? (
            ...
            <button onClick={handleImageUpload} disabled={!selectedImage || isProcessing} className="mt-4 w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2">
              {isProcessing ? (<><Loader2 className="w-5 h-5 animate-spin" />Extracting Text...</>) : (<>Extract Text from Image</>)}
            </button>
          </div>
        )}
      </div>
